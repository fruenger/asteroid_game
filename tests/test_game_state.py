...
class TestPhaseAFlow(unittest.TestCase):
    ...
    def test_space_outside_window_rejected(self) -> None:
        ...
        self.assertEqual(self.gs.step, 0)
        self.assertEqual(self.gs.toast_message, TIME_WINDOW_REJECT_TOAST)
    ...
    def test_toast_in_tick_after_early_space(self) -> None:
        ...
        self.assertEqual(out.get("toast"), TIME_WINDOW_REJECT_TOAST)
    ...
    def test_telescope_aligns_after_integration(self) -> None:
        ...
        self.gs.ra_deg = 12.0
        self.gs.dec_deg = 5.0
        u = np.asarray(b1k.optical_axis_world_unit(self.gs.ra_deg, self.gs.dec_deg), dtype=np.float64).reshape(3)
        self.gs.object_dir_cartesian = u / float(np.linalg.norm(u))
        keys = KeysInput()
    ...
    def test_three_exposures_complete(self) -> None:
        ...
        for _shot in range(3):
            ...
            for _ in range(16):
                ...
                wall += 0.5
                if not self.gs.step4_exposure_active:
                    break
            self.gs.tick(dt=0.01, wall_t=wall, keys=KeysInput(held=KeysHeld(space=False)), hints=FrameHints())
            wall += 0.01
    ...
    def test_step6_save_appends_catalog_line(self) -> None:
        ...
        os.close(fd)
        try:
            with mock.patch.dict(os.environ, {"B1_USER_SAVES_PATH": path}):
